            ...
            for j = 0, 15 do
                for i = 0, 15 do
                    ...
                    if not is_wet(i, j) and mget(i, j) ~= 0 then
						...
						if c_tile == 10 then
							if (c_to_down and (is_wet(i, j + 1) or is_cross_verti_wet(i, j + 1)) ) or (c_to_top and (is_wet(i, j - 1) or is_cross_verti_wet(i, j - 1))) and (not c_to_left or (c_to_left and not is_wet(i-1, j))) and (not c_to_right or (c_to_right and not is_wet(i+1, j))) then
								mset(i, j, pipes[pipes[mget(i, j)] ].tile_wet_verti)
								updating = true
							end
							
							if (c_to_left and (is_wet(i - 1, j) or is_cross_hori_wet(i - 1, j)) ) or (c_to_right and (is_wet(i + 1, j) or is_cross_hori_wet(i + 1, j)) ) and (not c_to_down or (c_to_down and not is_wet(i, j+1))) and (not c_to_top or (c_to_top and not is_wet(i, j - 1))) then
